---
tags:
  - task-management
  - dataview
  - automation
  - dashboard
  - Type/Index
  - Domain/Cognitive
  - task-management
  - dataview
  - automation
  - dashboard
---
# 智能任务管理仪表板

## 一、全局任务概览

<%*
// 基础任务统计
const allTasks = dv.pages()
    .where(p => p.tasks && p.tasks.length > 0)
    .flatMap(p => p.tasks.map(t => ({
        ...t,
        page: p,
        projectName: p.tags?.find(t => t.includes('project')) || '未分类'
    })));

const completedTasks = allTasks.filter(t => t.completed);
const activeTasks = allTasks.filter(t => !t.completed);
const overdueTasks = allTasks.filter(t => t.due && t.due < Date.now() && !t.completed);
%>

### 📊 任务统计概览

| 指标 | 数量 | 比例 |
|:---|:---|:---|
| **总任务** | <%= allTasks.length %> | 100% |
| **已完成** | <%= completedTasks.length %> | <%= ((completedTasks.length / allTasks.length) * 100).toFixed(1) %>% |
| **进行中** | <%= activeTasks.length %> | <%= ((activeTasks.length / allTasks.length) * 100).toFixed(1) %>% |
| **已逾期** | <%= overdueTasks.length %> | <%= ((overdueTasks.length / allTasks.length) * 100).toFixed(1) %>% |

---

## 二、今日推荐任务

<%*
// 智能任务推荐算法
const today = Date.now();
const oneDay = 24 * 60 * 60 * 1000;

// 筛选今日应该完成的任务
const recommendedTasks = activeTasks
    .filter(t => {
        // 1. 紧急任务（今天或明天到期）
        if (t.due) {
            const daysUntil = (t.due - today) / oneDay;
            if (daysUntil <= 1) return true;
        }

        // 2. 高优先级任务
        if (t.priority === 'high' || t.priority === 'urgent') return true;

        // 3. 阻塞其他任务的任务
        if (t.blocks && t.blocks.length > 0) return true;

        return false;
    })
    .sort((a, b) => {
        // 按截止日期优先排序
        if (a.due && b.due) return a.due - b.due;
        if (a.due) return -1;
        if (b.due) return 1;

        // 按优先级排序
        const priorityOrder = { 'urgent': 0, 'high': 1, 'medium': 2, 'low': 3 };
        return (priorityOrder[a.priority] || 4) - (priorityOrder[b.priority] || 4);
    })
    .slice(0, 10); // 最多显示10个任务
%>

### 🎯 今日重点任务（<%= recommendedTasks.length %>项）

<%*
if (recommendedTasks.length === 0) {
%>
> ✅ **恭喜！今日没有紧急任务**
> 您可以专注于长期规划或深度工作。

<%
} else {
    recommendedTasks.forEach(task => {
%>
<%= task.completed ? '✅' : '⬜' %> [[<%= task.page.file.name %>]] - <%= task.text %>
<%
        if (task.due) {
            const daysUntil = Math.ceil((task.due - today) / oneDay);
            const urgencyClass = daysUntil <= 0 ? 'text-red-600' : daysUntil <= 1 ? 'text-orange-600' : 'text-blue-600';
%>
<span class="<%= urgencyClass %>">📅 <%= daysUntil <= 0 ? '已逾期' : daysUntil === 0 ? '今天' : daysUntil + '天后' %></span>
<%
        }
        if (task.priority) {
            const priorityIcon = {
                'urgent': '🔴',
                'high': '🟠',
                'medium': '🟡',
                'low': '🟢'
            }[task.priority] || '⚪';
%>
<span><%= priorityIcon %> <%= task.priority.toUpperCase() %></span>
<%
        }
%>

<%
    });
}
%>

---

## 三、项目进度跟踪

<%*
// 按项目分组任务
const tasksByProject = {};
activeTasks.forEach(task => {
    const project = task.page.tags?.find(t => t.includes('project'))?.split('/').pop() || '未分类';
    if (!tasksByProject[project]) {
        tasksByProject[project] = { active: 0, completed: 0, total: 0 };
    }
    tasksByProject[project].active++;
    tasksByProject[project].total++;
});

completedTasks.forEach(task => {
    const project = task.page.tags?.find(t => t.includes('project'))?.split('/').pop() || '未分类';
    if (!tasksByProject[project]) {
        tasksByProject[project] = { active: 0, completed: 0, total: 0 };
    }
    tasksByProject[project].completed++;
    tasksByProject[project].total++;
});
%>

### 📈 项目进度概览

| 项目 | 活跃任务 | 已完成 | 总数 | 完成率 | 进度 |
|:---|:---|:---|:---|:---|:---|
<%*
Object.entries(tasksByProject)
    .sort((a, b) => b[1].total - a[1].total)
    .forEach(([project, stats]) => {
        const completionRate = ((stats.completed / stats.total) * 100).toFixed(1);
        const progressBar = '█'.repeat(Math.floor(completionRate / 10)) + '░'.repeat(10 - Math.floor(completionRate / 10));
%>
| <%= project %> | <%= stats.active %> | <%= stats.completed %> | <%= stats.total %> | <%= completionRate %>% | <%= progressBar %> |
<%
    });
%>

---

## 四、风险预警

<%*
// 风险分析
const riskTasks = activeTasks.filter(t => {
    // 1. 逾期任务
    if (t.due && t.due < today) return true;

    // 2. 即将到期的高优先级任务
    if (t.due && (t.priority === 'urgent' || t.priority === 'high')) {
        const daysUntil = (t.due - today) / oneDay;
        if (daysUntil <= 3) return true;
    }

    return false;
});

// 工作负载分析
const thisWeekTasks = activeTasks.filter(t => {
    if (!t.estimated) return false;
    const hours = parseInt(t.estimated.match(/(\d+)h/)?.[1] || 0);
    return hours > 0;
});

const totalEstimatedHours = thisWeekTasks.reduce((sum, t) => {
    const hours = parseInt(t.estimated?.match(/(\d+)h/)?.[1] || 0);
    return sum + hours;
}, 0);
%>

### ⚠️ 风险分析

#### 逾期任务 (<%= overdueTasks.length %>项)
<%*
if (overdueTasks.length === 0) {
%>
✅ **无逾期任务**

<%
} else {
    overdueTasks.slice(0, 5).forEach(task => {
%>
- [[<%= task.page.file.name %>]] - <%= task.text %>
<%
    });
    if (overdueTasks.length > 5) {
%>
... 还有 <%= overdueTasks.length - 5 %> 项逾期任务
<%
    }
}
%>

#### 工作负载预警
<%*
if (totalEstimatedHours > 40) {
%>
🔴 **工作负载过高**
- 本周预估工时：<%= totalEstimatedHours %>小时
- 建议优化优先级或重新规划任务

<%
} else if (totalEstimatedHours > 30) {
%>
🟠 **工作负载较高**
- 本周预估工时：<%= totalEstimatedHours %>小时
- 需要关注任务进度

<%
} else if (totalEstimatedHours > 0) {
%>
🟢 **工作负载正常**
- 本周预估工时：<%= totalEstimatedHours %>小时

<%
} else {
%>
⚪ **工作负载未明确**
- 建议为任务添加工时估算

<%
}
%>

---

## 五、智能建议

<%*
// 生成智能建议
const suggestions = [];

// 1. 逾期任务处理建议
if (overdueTasks.length > 0) {
    suggestions.push({
        type: 'urgent',
        icon: '🚨',
        message: `立即处理 ${overdueTasks.length} 项逾期任务，或重新评估优先级和截止日期`
    });
}

// 2. 工作负载优化建议
if (totalEstimatedHours > 40) {
    suggestions.push({
        type: 'warning',
        icon: '📊',
        message: `工作负载过高（${totalEstimatedHours}h），建议将 ${totalEstimatedHours - 40}h 的任务延后或重新分配`
    });
} else if (totalEstimatedHours < 20) {
    suggestions.push({
        type: 'info',
        icon: '💡',
        message: `工作负载较低（${totalEstimatedHours}h），可以考虑添加更多任务或专注于深度工作`
    });
}

// 3. 高优先级任务建议
const highPriorityTasks = activeTasks.filter(t =>
    t.priority === 'urgent' || t.priority === 'high'
);
if (highPriorityTasks.length > 5) {
    suggestions.push({
        type: 'warning',
        icon: '⚠️',
        message: `高优先级任务过多（${highPriorityTasks.length}项），建议重新评估紧急程度`
    });
}

// 4. 依赖关系建议
const blockedTasks = activeTasks.filter(t => t.blocked);
if (blockedTasks.length > 0) {
    suggestions.push({
        type: 'info',
        icon: '🚧',
        message: `${blockedTasks.length} 项任务被阻塞，建议优先处理前置任务`
    });
}
%>

### 💡 智能建议

<%*
if (suggestions.length === 0) {
%>
✅ **当前状态良好**
没有明显的风险或需要优化的事项。

<%
} else {
    suggestions.forEach(suggestion => {
%>
<%= suggestion.icon %> **<%= suggestion.type.toUpperCase() %>**
<%= suggestion.message %>

<%
    });
}
%>

---

## 六、快速操作

### 🎯 快速任务创建

<%*
// 快速任务创建模板
const newTaskTemplate = {
    due: tp.date.now("YYYY-MM-DD"),
    created: tp.date.now(),
    priority: "medium",
    status: "todo"
};
%>

```javascript
// 创建新任务
<%*
const taskText = await tp.system.prompt("输入任务描述：");
if (taskText) {
%>
- [ ] <%= taskText %> 📅 <%= newTaskTemplate.due %> #priority/<%= newTaskTemplate.priority %> #status/<%= newTaskTemplate.status %>
<%
}
%>
```

### 🔄 批量操作

- [ ] **归档已完成任务**
- [ ] **重置逾期任务优先级**
- [ ] **优化本周工作计划**
- [ ] **生成Sprint报告**

---

## 七、个性化设置

### 显示配置

```yaml
dashboard_settings:
  show_completed: true
  max_active_tasks: 15
  group_by_project: true
  show_risk_analysis: true
  enable_suggestions: true
  working_hours_per_day: 8
  working_days_per_week: 5
```

### 优先级配置

```yaml
priority_settings:
  urgent:
    icon: "🔴"
    weight: 10
    auto_escalation: true
  high:
    icon: "🟠"
    weight: 8
    auto_escalation: false
  medium:
    icon: "🟡"
    weight: 5
    auto_escalation: false
  low:
    icon: "🟢"
    weight: 2
    auto_escalation: false
```

---

**最后更新**：<%= tp.date.now("YYYY-MM-DD HH:mm") %>
**下次自动刷新**：<%= tp.date.now("add", 1, "days", "YYYY-MM-DD HH:mm") %>
