...
### 自动化健康监控
知识库配置了自动化健康监控系统 (`5.Misc/Template/Scripts/vault_health_monitor.py`):

**监控维度** (7大维度):
1. **孤立笔记检测**: 识别链接数 < 2 的笔记
2. **索引覆盖率**: 检查目录索引完整性
3. **标签规范性**: 验证标签使用是否符合三层标签体系
4. **文件健康度**: 检查空笔记、过时内容、低价值文件
5. **链接完整性**: 验证内部链接的有效性
6. **元数据完整性**: 检查 YAML frontmatter 的完整性
7. **内容新鲜度**: 识别长期未更新的内容

**技术实现**:
- Python 脚本自动扫描整个知识库
- 生成每日健康报告到 `0.DailyNotes/`
- 支持预警机制 (健康度 < 75% 时触发警告)
- 可集成到 Templater 工作流
