...
- **我的作品构建记录与行动项**:
  - [ ] 学习 RAG 技术原理：
    - [ ] 阅读核心概念文章 [[RAG 技术学习资源]]
    - [ ] 完成入门教程 [在此记录完成的教程链接或笔记]
    - [在此记录学习过程中的核心概念理解或疑问]
  - [ ] 搭建 LangChain + ChromaDB 环境：
    - [ ] 完成环境安装与基础配置 [[LangChain 快速入门]]
    - [ ] 编写测试代码，验证环境可用性 [链接到对应的代码片段笔记]
    - [在此记录环境搭建遇到的问题和解决方案]
  - [ ] 尝试为 Obsidian 插件文档构建问答助手：
    - [ ] 收集并处理文档数据 [[Obsidian 插件文档]] [记录数据来源和处理过程]
    - [ ] 构建向量索引 [链接到构建索引的代码或脚本笔记]
    - [ ] 实现问答接口或命令行工具 [链接到问答系统代码库]
    - [ ] 测试问答效果，并进行优化 [记录测试结果和优化方向]
  - **我的实践成果与反思**:
...
- **关键步骤**:
  ...
  2. 设计一种表达任务流程和网页交互规则的结构化方式（如 YAML/JSON 配置文件，或设计 Prompt 指导 LLM 理解网页并生成行动序列）。
