...
> ```
> 
> 保存并退出编辑器后，Git会将这些提交压缩成一个，并提示你输入一个新的提交消息。
> 
> 通过这种方式，你可以将多个提交合并为一个，使得项目的历史更加清晰和易于理解。这对于准备将功能分支合并到主分支时特别有用，因为它允许你以更整洁的方式呈现你的更改。
...
1. **别名`ac`**：创建一个名为`ac`的别名，用于快速执行`add`和`commit`命令。
2. **修改提交**：使用`git commit --amend`来修改最近一次提交的消息。
3. **基于浏览器的编辑**：在没有本地计算机的情况下，通过浏览器使用GitHub编辑器。
4. **GitHub代码空间**：在云中设置代码空间，提供VS Code的全部功能。
5. **Git Stash**：使用`git stash`保存和恢复更改，如`git stash save "message"`和`git stash pop`。
6. **更改默认分支名**：从`master`更改为`main`或其他名称。
7. **查看提交历史**：使用`git log --oneline --graph`查看简化的提交历史。
8. **二分查找问题提交**：使用`git bisect`查找破坏应用程序的提交。
9. **压缩提交**：使用`git rebase -i`将多个提交压缩成一个。

这些笔记简洁且易于阅读，重点关注可能在测验中出现的关键概念和例子。
